...
<div class="card">
    ...
    <div class="sub">Customer Payment Confirmation</div>
    <div class="info">
        <?php $c = $customer; ?>
        <div><strong>Customer:</strong> <?= htmlspecialchars((string)($c['cus_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?></div>
        <div><strong>Contact:</strong>  <?= htmlspecialchars((string)($c['cus_mobile'] ?? ''), ENT_QUOTES, 'UTF-8') ?></div>
        ...
        <?php endif; ?>
        <div><strong>Balance:</strong>  Rs. <?= number_format((float)($c['accbalance'] ?? 0), 2) ?></div>
    ...
    <div class="btn-row">
        ...
        <button class="btn-skip"  onclick="window.location.href='/finance/customer-payments'">✖ No Need</button>
    </div>
</div>
...
function doPrinting() {
    ...
    $cusName  = addslashes((string)($customer['cus_name'] ?? ''));
    $cusTp    = addslashes((string)($customer['cus_mobile'] ?? ''));
    ...
    var escposCommands = doc
        ...
        .text("Customer Payment Confirmation")
        .feed(1)
    ...
    sendEscposJob(escposCommands, printerName, function() {
        window.location.href = '/finance/customer-payments';
    });
}
</script>
</body>
