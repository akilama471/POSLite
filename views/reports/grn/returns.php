<?php // views/reports/grn/returns.php — rpt_grn_returnlist.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Stock Return List</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_grn_returnlist.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Stock Return List</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Return Ref</th><th>Date</th><th>GRN Ref</th><th>Supplier</th><th style="text-align:center;">Items</th><th style="text-align:right;">Value (Rs)</th><th>Status</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','grn_return_list');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="7" class="text-center muted">No return records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.return_refno||'')}</td><td>${esc(r.return_date||'')}</td><td>${esc(r.grn_refno||'')}</td><td>${esc(r.suppler_name||'—')}</td><td style="text-align:center;">${r.item_count||0}</td><td style="text-align:right;">${parseFloat(r.total_value||0).toFixed(2)}</td><td>${esc(r.status_label||'')}</td>`;tbody.appendChild(tr);});
}
