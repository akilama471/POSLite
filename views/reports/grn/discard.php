<?php // views/reports/grn/discard.php — rpt_grn_returndiscard.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Discard Log</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_grn_returndiscard.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Discard Log</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Date</th><th>Item Name</th><th>IMEI / Part</th><th>Operator</th><th>Reason</th><th style="text-align:center;">Qty</th><th style="text-align:right;">Cost (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','grn_discard_log');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="7" class="text-center muted">No discard records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.discard_date||'')}</td><td>${esc(r.item_name||'')}</td><td>${esc(r.imei_no||'—')}</td><td>${esc(r.operator_name||'—')}</td><td>${esc(r.discard_reason||'')}</td><td style="text-align:center;">${r.qty||0}</td><td style="text-align:right;">${parseFloat(r.item_cost||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
