<?php // views/reports/grn/sales_return_bin.php — rpt_grn_returnagainstock.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Customer Sales Return Bin</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_grn_returnagainstock.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Customer Sales Return Bin</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Return Date</th><th>Bill No</th><th>Customer</th><th>Item</th><th>IMEI/Part</th><th style="text-align:center;">Qty</th><th style="text-align:right;">Return Value (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','grn_sales_return_bin');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="7" class="text-center muted">No return bin records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.return_date||'')}</td><td>${esc(r.billnumber||'')}</td><td>${esc(r.cus_name||'Walk-In')}</td><td>${esc(r.item_name||'')}</td><td>${esc(r.imei_part_no||'—')}</td><td style="text-align:center;">${r.return_qty||0}</td><td style="text-align:right;">${parseFloat(r.return_value||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
