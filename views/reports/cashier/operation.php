<?php // views/reports/cashier/operation.php — rpt_cashier_operation.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Cashier Operation Log</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_cashier_operation.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Cashier Operation Log</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Slot</th><th>Operator</th><th>Open Time</th><th>Close Time</th><th style="text-align:right;">Cash Open (Rs)</th><th style="text-align:right;">Cash Close (Rs)</th><th style="text-align:right;">Card Open (Rs)</th><th style="text-align:right;">Card Close (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','cashier_operation_log');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="8" class="text-center muted">No records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.operation_slot||'')}</td><td>${esc(r.operator_name||'—')}</td><td>${esc(r.recordtime||'')}</td><td>${esc(r.close_time||'Still Open')}</td><td style="text-align:right;">${parseFloat(r.cash_openbal||0).toFixed(2)}</td><td style="text-align:right;">${parseFloat(r.cash_closebal||0).toFixed(2)}</td><td style="text-align:right;">${parseFloat(r.card_openbal||0).toFixed(2)}</td><td style="text-align:right;">${parseFloat(r.card_closebal||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
