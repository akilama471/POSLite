<div class="app-shell">
    <header class="topbar">
        <div>
            <div class="brand">Shop Open Close Balance</div>
            <div class="muted" style="color: #b8c6cf;">Migrated from `rpt_cashier_openclose.php`</div>
        </div>
    </header>
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none; min-height: 400px;">
                <div style="text-align: center; margin-bottom: 24px;">
                    <h2 style="margin-bottom: 4px;">Shop Open Close Balance Details</h2>
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%; text-align: right;">Open Balance</th>
                            <th style="width: 15%; text-align: right;">Close Balance</th>
                        </tr>
                    </thead>
...
async function loadReport() {
    ...
    try {
        ...
        formData.append('type', 'cashier_openclose');
...
function renderTable(data, from, to) {
    ...
    subtitle.textContent = `Open/Close details from ${from} to ${to}`;
    ...
    data.forEach(row => {
        ...
        const opn = parseFloat(row.open_balance) || 0;
        const cls = parseFloat(row.close_balance) || 0;

        tr.innerHTML = `
            <td>${escapeHtml(row.op_date)}</td>
            <td>${escapeHtml(row.shop_info_name || 'System')}</td>
            <td>${escapeHtml(row.remark)}</td>
            <td style="text-align: right;">Rs. ${opn.toFixed(2)}</td>
            <td style="text-align: right;">Rs. ${cls.toFixed(2)}</td>
        `;
