<?php // views/reports/cashier/cashin.php — rpt_cashier_cashin.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Cash-In Report</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_cashier_cashin.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Cash-In Report</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Date</th><th>Account</th><th>Operator</th><th>Remark</th><th style="text-align:right;">Amount (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','cashier_cashin');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.record_time||'')}</td><td>${esc(r.acc_name||'—')}</td><td>${esc(r.operator_name||'—')}</td><td>${esc(r.remark||'')}</td><td style="text-align:right;">${parseFloat(r.amount||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
