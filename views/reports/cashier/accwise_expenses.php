<?php // views/reports/cashier/accwise_expenses.php — rpt_cashier_accwiseexpences.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Account-Wise Expenses Report</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_cashier_accwiseexpences.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Account-Wise Expenses</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Account</th><th style="text-align:center;">Count</th><th style="text-align:right;">Total Expenses (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','cashier_accwise_expenses');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="3" class="text-center muted">No records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.acc_name||'—')}</td><td style="text-align:center;">${r.count||0}</td><td style="text-align:right;">${parseFloat(r.total_amount||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
