<?php
// views/reports/sales/bestsale.php — rpt_sale_bestsale.php
?>
<div class="app-shell">
    <header class="topbar">
        <div>
            <div class="brand">Best Selling Items Report</div>
            <div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_sale_bestsale.php</code></div>
        </div>
    </header>
        ...
        <main class="page">
                ...
                <div class="grid" style="grid-template-columns:repeat(3,1fr) auto;gap:16px;align-items:end;">
                    ...
                    <div class="form-row" style="margin:0;">
                        ...
                        <select class="input" id="shop_id">
                            ...
                            <?php foreach ($shops as $s): ?><?php if ($s['shopid'] > 0): ?>
                            <option value="<?= (int)$s['shopid'] ?>"><?= htmlspecialchars((string)$s['shop_info_name'], ENT_QUOTES, 'UTF-8') ?></option>
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;">
                    <h2 style="margin-bottom:4px;">Best Selling Items</h2>
                    <div class="muted" id="report-subtitle"></div>
                </div>
                <table class="data-table">
                    <thead><tr><th>#</th><th>Item Name</th><th>Part / IMEI</th><th style="text-align:center;">Qty Sold</th><th style="text-align:right;">Total Revenue (Rs)</th></tr></thead>
...
async function loadReport() {
    ...
    fd.append('type','best_sale'); fd.append('from_date',document.getElementById('from_date').value);
    ...
    json.data.forEach((r,i)=>{
        ...
        tr.innerHTML=`<td>${i+1}</td><td>${esc(r.item_name)}</td><td>${esc(r.imei_part_no||'—')}</td><td style="text-align:center;">${r.total_qty||0}</td><td style="text-align:right;">${parseFloat(r.total_value||0).toFixed(2)}</td>`;
