<?php // views/reports/sales/overcost.php — rpt_sale_overcost.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Over-Cost Sales Report</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_sale_overcost.php</code></div></div></header>
    <div class="shell-grid">
        ...
        <main class="page">
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Over-Cost Sales</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Bill No</th><th>Date</th><th>Item Name</th><th style="text-align:right;">Cost (Rs)</th><th style="text-align:right;">Sale Price (Rs)</th><th style="text-align:right;">Over-Cost (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','over_cost_sale');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="6" class="text-center muted">No over-cost records found.</td></tr>';return;}
    json.data.forEach(r=>{const diff=parseFloat(r.sale_price||0)-parseFloat(r.cost_price||0);const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.billnumber)}</td><td>${esc(r.sale_date||'')}</td><td>${esc(r.item_name)}</td><td style="text-align:right;">${parseFloat(r.cost_price||0).toFixed(2)}</td><td style="text-align:right;">${parseFloat(r.sale_price||0).toFixed(2)}</td><td style="text-align:right;color:#f87171;">${diff.toFixed(2)}</td>`;tbody.appendChild(tr);});
}
