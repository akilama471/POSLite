<?php // views/reports/sales/phonesale.php — rpt_sale_phonesale.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Phone / IMEI Sale Report</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_sale_phonesale.php</code></div></div></header>
        ...
        <main class="page">
            <section class="card stack" style="margin-bottom:24px;">
                <div class="grid" style="grid-template-columns:repeat(4,1fr) auto;gap:16px;align-items:end;">
                    ...
                    <div class="form-row" style="margin:0;"><label for="imei_search">IMEI / Part No (optional)</label><input type="text" class="input" id="imei_search" placeholder="Search IMEI..."></div>
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2>Phone / IMEI Sale Report</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Bill No</th><th>Date</th><th>Customer</th><th>Item</th><th>IMEI</th><th style="text-align:right;">Sale Price (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    const fd=new FormData();fd.append('type','phone_sale');fd.append('from_date',document.getElementById('from_date').value);fd.append('to_date',document.getElementById('to_date').value);fd.append('shop_id',shopId);fd.append('imei',document.getElementById('imei_search').value);fd.append('_token','<?=htmlspecialchars(csrf_token(),ENT_QUOTES,'UTF-8')?>');
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="6" class="text-center muted">No records found.</td></tr>';return;}
    json.data.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(r.billnumber)}</td><td>${esc(r.sale_date||r.billaddedtime||'')}</td><td>${esc(r.cus_name||'Walk-In')}</td><td>${esc(r.item_name)}</td><td>${esc(r.imei_part_no||'—')}</td><td style="text-align:right;">${parseFloat(r.sale_price||0).toFixed(2)}</td>`;tbody.appendChild(tr);});
}
