<?php // views/reports/sales/itemwise.php — rpt_sale_itemwisesale.php ?>
<div class="app-shell">
    <header class="topbar"><div><div class="brand">Item Wise Sale Report</div><div class="muted" style="color:#b8c6cf;">Migrated from <code>rpt_sale_itemwisesale.php</code></div></div></header>
        ...
        <main class="page">
            <section class="card stack" style="margin-bottom:24px;">
                <div class="grid" style="grid-template-columns:repeat(4,1fr) auto;gap:16px;align-items:end;">
                    ...
                    <div class="form-row" style="margin:0;"><label for="item_name">Item Name (optional)</label><input type="text" class="input" id="item_name" placeholder="Search item..."></div>
            ...
            <section class="card stack print-area" id="report-container" style="display:none;">
                <div style="text-align:center;margin-bottom:24px;"><h2 style="margin-bottom:4px;">Item Wise Sale Report</h2><div class="muted" id="report-subtitle"></div></div>
                <table class="data-table">
                    <thead><tr><th>Bill No</th><th>Date</th><th>Item Name</th><th>Part/IMEI</th><th style="text-align:center;">Qty</th><th style="text-align:right;">Price (Rs)</th><th style="text-align:right;">Total (Rs)</th></tr></thead>
...
async function loadReport(){
    ...
    if(shopId==0){alert('Please select a shop.');return;}
    const fd=new FormData();
    fd.append('type','item_wise_sale');fd.append('from_date',document.getElementById('from_date').value);
    ...
    if(!json.data.length){tbody.innerHTML='<tr><td colspan="7" class="text-center muted">No records found.</td></tr>';return;}
    json.data.forEach(r=>{
        const tr=document.createElement('tr');
        tr.innerHTML=`<td>${esc(r.billnumber)}</td><td>${esc(r.sale_date||r.billaddedtime||'')}</td><td>${esc(r.item_name)}</td><td>${esc(r.imei_part_no||'—')}</td><td style="text-align:center;">${r.qty||0}</td><td style="text-align:right;">${parseFloat(r.sale_price||0).toFixed(2)}</td><td style="text-align:right;">${parseFloat(r.sub_total||0).toFixed(2)}</td>`;
