...
$text = (string) ($_GET["text"] ?? "0");
$size = max(20, (int) ($_GET["size"] ?? 32));
$printText = isset($_GET["print"]) && $_GET["print"] === "true";

render_code128_barcode($text, $size, $printText);

function render_code128_barcode(string $text, int $size, bool $printText): void
{
    $patterns = [
    ...
    foreach (str_split($text) as $index => $character) {
        ...
        $encoded .= $patterns[$character];
        $checksum += $values[$character] * ($index + 1);
    }

    $encoded .= $patterns[$keys[$checksum % 103]];
    $encoded = "211214" . $encoded . "2331112";

    $length = 20;
    foreach (str_split($encoded) as $unit) {
        $length += (int) $unit;
    }

    $barWidth = 1;
    $textHeight = $printText ? 15 : 5;
    $width = max(1, $length * $barWidth);
    ...
    if ($printText) {
        imagestring($image, 2, 10, $size, $text, $black);
    }

    $position = 10;
    foreach (str_split($encoded) as $index => $unit) {
        $next = $position + (int) $unit;
