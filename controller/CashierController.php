...
class CashierController
{
    public function index(Request $request): void
    {
        ...
        // Build close-duty session summary when duty is active
        $sessionSummary = null;
        $shopUsers      = [];
        if ($context['isActive'] && $context['slot'] !== null) {
            $slotId         = (int)$context['slot']['recordid'];
            $sessionSummary = $cashierModel->getSessionSummary($slotId, $userId, $shopId);
            $shopUsers      = $cashierModel->listShopUsers($shopId);
        }
    ...
    public function close(Request $request): void
    {
        ...
        // close_type: 1 = keep with current user, 2 = transfer slot to another operator
        $closeType      = (int)$request->input('close_type', 1);
        $transferToUser = (int)$request->input('transfer_to_user', 0);
        ...
        $slotId         = (int)$context['slot']['recordid'];
        $transferTarget = ($closeType === 2 && $transferToUser > 0) ? $transferToUser : 0;

        $cashierModel->closeDutyWithTransfer($slotId, $userId, $cashClose, $cardClose, $transferTarget);
        sync_cashier_session(false, null);

        $msg = $transferTarget > 0
            ? 'Cashier duty closed and slot transferred successfully.'
            : 'Cashier duty closed successfully.';
