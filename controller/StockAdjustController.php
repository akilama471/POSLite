...
class StockAdjustController
{
    public function index(Request $request): void
    {
        ...
        View::make("stock/adjust/index", [
            "title" => "Stock Adjustment",
            "auth" => $auth,
            "categories" => $categoryModel->allOrdered(),
            "candidates" => $_SESSION["stock_adjust_candidates"] ?? [],
            "search" => $_SESSION["stock_adjust_search"] ?? ["mode" => "code", "query" => ""],
            "flash" => $_SESSION["flash"] ?? null,
        ]);

        unset($_SESSION["flash"], $_SESSION["stock_adjust_candidates"], $_SESSION["stock_adjust_search"]);
    }

    public function search(Request $request): void
    {
        if (!$this->validCsrf($request, "/stock/adjust")) {
            return;
        }
        ...
        $model = new StockAdjust();

        $_SESSION["stock_adjust_search"] = [
            "mode" => $mode,
            "query" => $query,
        ];
    ...
    public function submit(Request $request): void
    {
        ...
        try {
            (new StockAdjust())->adjustStock(
                auth_user() ?? [],
                (int) $request->input("object_type", 0),
                (int) $request->input("row_id", 0),
                (int) $request->input("qty_diff", 0),
                trim((string) $request->input("reason", ""))
            );
            $_SESSION["flash"] = ["type" => "success", "message" => "Stock adjusted successfully."];
        } catch (Throwable $exception) {
            $_SESSION["flash"] = ["type" => "error", "message" => $exception->getMessage()];
        }
